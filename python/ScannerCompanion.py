...
def buildCamera():
    ...
    #camera.iso = 200 #Higher value is brighter
    #camera.awb_mode = 'auto'
    #camera.framerate= Fraction(2,1)
    #camera.shutter_speed = 500000
    #camera.exposure_mode = 'verylong'
    #camera.drc_strength = 'medium' # Dynamic Range Compression (overrides awb_mode and awb_gains)
    #camera.rotation = 180 #Turns the image upside down
...
def changeCyclesRotation(slider_value):
    global cyclesRotation
    print("Your turntable will rotate in " + str(cyclesRotation * 0.5) + " degree increments")
    cyclesRotation = int(slider_value) * 0.5# This value maps the turntable steps to = degrees
...
def shoot(): #PiCam - capture single photo
    ...
    if (shotNumber < 10):
        camera.capture(path + "/0" + str(shotNumber) + str(imageExtension), str(imageOutputType))
    else:
        camera.capture(path + "/" + str(shotNumber) + str(imageExtension), str(imageOutputType))
    shotNumber += 1
...
PushButton(turntableBox, command=driveCW, text="Turntable CW", grid=[1,0])
PushButton(turntableBox, command=driveCCW, text="Turntable CCW", grid=[0,0])
